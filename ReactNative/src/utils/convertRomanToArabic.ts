import { romanNumDict } from "../constants";

const convertNumFromRomanToArabic = (givenRomanNumStr: string) => {
  // this fn recieve a str, return a number
  if (!givenRomanNumStr.length || !givenRomanNumStr) {
    return "";
  }

  const validRomanNumRegex =
    /^M{0,3}(C[MD]|D?C{0,3})(X[CL]|L?X{0,3})(I[XV]|V?I{0,3})$/g;
...
const convertExpFromRomanToArabic = (givenExp: string) => {
  ...
  for (let i = 0; i < givenExp.length; i++) {
    if (numRegex.test(givenExp[i])) {
      ...
      if (i === givenExp.length - 1 && numStr.length > 0) {
        let arabicNum: any = convertNumFromRomanToArabic(numStr);
